...
export const Login = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // The interceptor in client.ts returns response.data.data if status === 'success'
      const response: any = await apiClient.post('/auth/login', { username, password });
      
      const accessToken = response?.accessToken;

      if (!accessToken) {
        throw new Error('Server response is invalid: Missing token.');
      }

      // Decode JWT to get user info (id, role, etc)
      ...
      if (user.role !== 'SUPER_ADMIN' && user.role !== 'ADMIN') {
        throw new Error('This panel is restricted to system administrators.');
      }
            ...
            <div>
              ...
              <div className="mt-1 relative rounded-md shadow-sm">
                ...
                <input
                  ...
                  className="pl-10 block w-full sm:text-sm border-gray-300 rounded-md focus:ring-primary-500 focus:border-primary-500 py-3 border"
